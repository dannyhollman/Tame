...
public class TamePandaEntity extends TameableEntity {
	
	private static final DataParameter<Integer> UNHAPPY_COUNTER = EntityDataManager.createKey(TamePandaEntity.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> SNEEZE_COUNTER = EntityDataManager.createKey(TamePandaEntity.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> EAT_COUNTER = EntityDataManager.createKey(TamePandaEntity.class, DataSerializers.VARINT);
	private static final DataParameter<Byte> MAIN_GENE = EntityDataManager.createKey(TamePandaEntity.class, DataSerializers.BYTE);
	private static final DataParameter<Byte> HIDDEN_GENE = EntityDataManager.createKey(TamePandaEntity.class, DataSerializers.BYTE);
	private static final DataParameter<Byte> PANDA_FLAGS = EntityDataManager.createKey(TamePandaEntity.class, DataSerializers.BYTE);
	...
	private TamePandaEntity.WatchGoal watchGoal;
	...
	public TamePandaEntity(EntityType<? extends TamePandaEntity> type, World worldIn) {
		super(type, worldIn);
		this.setTamed(false);
		this.moveController = new TamePandaEntity.MoveHelperController(this);
		if (!this.isChild()) {
	...
	public TamePandaEntity.Type getMainGene() {
		return TamePandaEntity.Type.byIndex(this.dataManager.get(MAIN_GENE));
	}

	public void setMainGene(TamePandaEntity.Type pandaType) {
		if (pandaType.getIndex() > 6) {
			pandaType = TamePandaEntity.Type.getRandomType(this.rand);
		}

		this.dataManager.set(MAIN_GENE, (byte)pandaType.getIndex());
	}

	public TamePandaEntity.Type getHiddenGene() {
		return TamePandaEntity.Type.byIndex(this.dataManager.get(HIDDEN_GENE));
	}

	public void setHiddenGene(TamePandaEntity.Type pandaType) {
		if (pandaType.getIndex() > 6) {
			pandaType = TamePandaEntity.Type.getRandomType(this.rand);
		}
	 ...
	 */
	 public void readAdditional(CompoundNBT compound) {
		...
		this.setMainGene(TamePandaEntity.Type.byName(compound.getString("MainGene")));
		this.setHiddenGene(TamePandaEntity.Type.byName(compound.getString("HiddenGene")));
	}


	@Nullable
	public AgeableEntity createChild(AgeableEntity ageable) {
		PandaEntity TamePandaEntity = EntityType.PANDA.create(this.world);
		TamePandaEntity.setAttributes();
		return TamePandaEntity;
	}

	protected void registerGoals() {
		this.goalSelector.addGoal(0, new SwimGoal(this));
		this.goalSelector.addGoal(2, new TamePandaEntity.PanicGoal(this, 2.0D));
		this.goalSelector.addGoal(2, new TamePandaEntity.MateGoal(this, 1.0D));
		this.goalSelector.addGoal(3, new TamePandaEntity.AttackGoal(this, (double)1.2F, true));
		this.goalSelector.addGoal(4, new TemptGoal(this, 1.0D, Ingredient.fromItems(Blocks.BAMBOO.asItem()), false));
		this.goalSelector.addGoal(5, new FollowOwnerGoal(this, 1.0D, 10.0F, 2.0F, false));
		this.goalSelector.addGoal(6, new TamePandaEntity.AvoidGoal<>(this, PlayerEntity.class, 8.0F, 2.0D, 2.0D));
		this.goalSelector.addGoal(6, new TamePandaEntity.AvoidGoal<>(this, MonsterEntity.class, 4.0F, 2.0D, 2.0D));
		this.goalSelector.addGoal(7, new TamePandaEntity.SitGoal());
		this.goalSelector.addGoal(8, new TamePandaEntity.LieBackGoal(this));
		this.goalSelector.addGoal(8, new TamePandaEntity.ChildPlayGoal(this));
		this.watchGoal = new TamePandaEntity.WatchGoal(this, PlayerEntity.class, 6.0F);
		this.goalSelector.addGoal(9, this.watchGoal);
		this.goalSelector.addGoal(10, new LookRandomlyGoal(this));
		this.goalSelector.addGoal(12, new TamePandaEntity.RollGoal(this));
		this.goalSelector.addGoal(13, new FollowParentGoal(this, 1.25D));
		this.goalSelector.addGoal(14, new WaterAvoidingRandomWalkingGoal(this, 1.0D));
		this.targetSelector.addGoal(1, (new TamePandaEntity.RevengeGoal(this)).setCallsForHelp(new Class[0]));
	}
	...
	public TamePandaEntity.Type func_213590_ei() {
		return TamePandaEntity.Type.func_221101_b(this.getMainGene(), this.getHiddenGene());
	}

	public boolean isLazy() {
		return this.func_213590_ei() == TamePandaEntity.Type.LAZY;
	}

	public boolean isWorried() {
		return this.func_213590_ei() == TamePandaEntity.Type.WORRIED;
	}

	public boolean isPlayful() {
		return this.func_213590_ei() == TamePandaEntity.Type.PLAYFUL;
	}

	public boolean isWeak() {
		return this.func_213590_ei() == TamePandaEntity.Type.WEAK;
	}

	public boolean isAggressive() {
		return this.func_213590_ei() == TamePandaEntity.Type.AGGRESSIVE;
	}
	...
	private void func_213577_ez() {
		...
		for(TamePandaEntity TamePandaEntity : this.world.getEntitiesWithinAABB(TamePandaEntity.class, this.getBoundingBox().grow(10.0D))) {
			if (!TamePandaEntity.isChild() && TamePandaEntity.onGround && !TamePandaEntity.isInWater() && TamePandaEntity.canPerformAction()) {
				TamePandaEntity.jump();
			}
		}
	...
	public ILivingEntityData onInitialSpawn(IWorld worldIn, DifficultyInstance difficultyIn, SpawnReason reason, @Nullable ILivingEntityData spawnDataIn, @Nullable CompoundNBT dataTag) {
		this.setMainGene(TamePandaEntity.Type.getRandomType(this.rand));
		this.setHiddenGene(TamePandaEntity.Type.getRandomType(this.rand));
		this.setAttributes();
		if (spawnDataIn == null) {
	...
	public void getGenesForChildFromParents(TamePandaEntity father, @Nullable TamePandaEntity mother) {
		if (mother == null) {
	...
	private TamePandaEntity.Type getOneOfGenesRandomly() {
	...
	static class AttackGoal extends MeleeAttackGoal {
		private final TamePandaEntity panda;

		public AttackGoal(TamePandaEntity pandaIn, double speedIn, boolean useLongMemory) {
	...
	static class AvoidGoal<T extends LivingEntity> extends AvoidEntityGoal<T> {
		...
		public AvoidGoal(TamePandaEntity pandaIn, Class<T> entityClassToAvoidIn, float distance, double nearSpeedIn, double farSpeedIn) {
	...
	static class ChildPlayGoal extends Goal {
		...
		public ChildPlayGoal(TamePandaEntity pandaIn) {
			this.panda = pandaIn;
		}
	...
	static class LieBackGoal extends Goal {
		...
		public LieBackGoal(TamePandaEntity pandaIn) {
			this.panda = pandaIn;
		}
	...
	class MateGoal extends BreedGoal {
		...
		public MateGoal(TamePandaEntity pandaIn, double speedIn) {
				...
				if (!this.func_220691_h()) {
					if (this.field_220694_f <= this.panda.ticksExisted) {
						...
						if (this.panda.isServerWorld()) {
							PlayerEntity playerentity = this.world.getClosestPlayer(TamePandaEntity.BREED_TARGETING, this.panda);
	...
	static class MoveHelperController extends MovementController {
		...
		public MoveHelperController(TamePandaEntity pandaIn) {
			super(pandaIn);
			this.panda = pandaIn;
		}
	...
	static class PanicGoal extends net.minecraft.entity.ai.goal.PanicGoal {
		...
		public PanicGoal(TamePandaEntity pandaIn, double speedIn) {
			super(pandaIn, speedIn);
			this.panda = pandaIn;
		}
	...
	static class RevengeGoal extends HurtByTargetGoal {
		...
		public RevengeGoal(TamePandaEntity pandaIn, Class<?>... p_i51462_2_) {
		...
		protected void setAttackTarget(MobEntity mobIn, LivingEntity targetIn) {
			if (mobIn instanceof TamePandaEntity && ((TamePandaEntity)mobIn).isAggressive()) {
	...
	static class RollGoal extends Goal {
		...
		public RollGoal(TamePandaEntity pandaIn) {
			this.panda = pandaIn;
	...
	class SitGoal extends Goal {
		 ...
		 */
		public boolean shouldExecute() {
			if (this.field_220832_b <= TamePandaEntity.this.ticksExisted && !TamePandaEntity.this.isChild() && !TamePandaEntity.this.isInWater() && TamePandaEntity.this.canPerformAction() && TamePandaEntity.this.getUnhappyCounter() <= 0) {
				List<ItemEntity> list = TamePandaEntity.this.world.getEntitiesWithinAABB(ItemEntity.class, TamePandaEntity.this.getBoundingBox().grow(6.0D, 6.0D, 6.0D), TamePandaEntity.PANDA_ITEMS);
				return !list.isEmpty() || !TamePandaEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND).isEmpty();
			} else {
				return false;
			}
		}
		 ...
		 */
		public boolean shouldContinueExecuting() {
			if (!TamePandaEntity.this.isInWater() && (TamePandaEntity.this.isLazy() || TamePandaEntity.this.rand.nextInt(600) != 1)) {
				return TamePandaEntity.this.rand.nextInt(2000) != 1;
			} else {
				return false;
			}
		}
		 ...
		 */
		public void tick() {
			if (!TamePandaEntity.this.func_213556_dX() && !TamePandaEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND).isEmpty()) {
				TamePandaEntity.this.tryToSit();
			}
		 ...
		 */
		public void startExecuting() {
			List<ItemEntity> list = TamePandaEntity.this.world.getEntitiesWithinAABB(ItemEntity.class, TamePandaEntity.this.getBoundingBox().grow(8.0D, 8.0D, 8.0D), TamePandaEntity.PANDA_ITEMS);
			if (!list.isEmpty() && TamePandaEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND).isEmpty()) {
				TamePandaEntity.this.getNavigator().tryMoveToEntityLiving(list.get(0), (double)1.2F);
			} else if (!TamePandaEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND).isEmpty()) {
				TamePandaEntity.this.tryToSit();
			}
		 ...
		 */
		public void resetTask() {
			ItemStack itemstack = TamePandaEntity.this.getItemStackFromSlot(EquipmentSlotType.MAINHAND);
			if (!itemstack.isEmpty()) {
				TamePandaEntity.this.entityDropItem(itemstack);
				TamePandaEntity.this.setItemStackToSlot(EquipmentSlotType.MAINHAND, ItemStack.EMPTY);
				int i = TamePandaEntity.this.isLazy() ? TamePandaEntity.this.rand.nextInt(50) + 10 : TamePandaEntity.this.rand.nextInt(150) + 10;
				this.field_220832_b = TamePandaEntity.this.ticksExisted + i * 20;
			}

			TamePandaEntity.this.func_213553_r(false);
		}
	}
	...
	public static enum Type {
		...
		private static final TamePandaEntity.Type[] field_221109_h = Arrays.stream(values()).sorted(Comparator.comparingInt(TamePandaEntity.Type::getIndex)).toArray((p_221102_0_) -> {
			return new TamePandaEntity.Type[p_221102_0_];
		});
		...
		private static TamePandaEntity.Type func_221101_b(TamePandaEntity.Type mainGene, TamePandaEntity.Type hiddenGene) {
		...
		public static TamePandaEntity.Type byIndex(int indexIn) {
		...
		public static TamePandaEntity.Type byName(String p_221108_0_) {
			for(TamePandaEntity.Type TamePandaEntity$type : values()) {
				if (TamePandaEntity$type.name.equals(p_221108_0_)) {
					return TamePandaEntity$type;
				}
			}
		...
		public static TamePandaEntity.Type getRandomType(Random randIn) {
	...
	static class WatchGoal extends LookAtGoal {
		private final TamePandaEntity field_220718_f;

		public WatchGoal(TamePandaEntity p_i51458_1_, Class<? extends LivingEntity> p_i51458_2_, float p_i51458_3_) {
